-- Fix: a customer added by the owner (via "Manage customers", which only
-- writes to the `customers` table, never `profiles`) got "Mobile number
-- is not registered." when trying to log in.
--
-- Root cause: `mvp_login_with_phone` only ever looked up the phone in
-- `profiles`. A customer added by the owner has no `profiles` row yet
-- (that row is only created on their first successful login), so the
-- lookup always missed and the function raised "not registered" even
-- though the phone genuinely exists as a customer.
--
-- This is already fixed in supabase/schema.sql (the function falls back
-- to looking the phone up in `customers` when it's missing from
-- `profiles`), but since this project has no migration runner, that fix
-- has to be applied to the live database by hand. This file re-applies
-- that exact function definition -- safe to run even if it's already
-- deployed, since `create or replace` is idempotent.
